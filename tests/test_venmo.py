...
def _one_month(*rows: str) -> str:
    """A minimal statement carrying just the rows a test cares about."""
    return "\n".join(
        [
            "Account Statement - (@jordan-mercer) - January 2026",
            "Jordan Mercer",
...
def test_merchant_carries_the_note_so_a_series_says_what_it_is_for():
    # Paying one person for several things is normal, so the counterparty
    # alone cannot identify what recurs: "Alex Liang, $35 monthly" tells the
    # user nothing. Folding the note in separates a phone bill from a dinner,
    # and ad-hoc notes are all different so they form no series — which is the
    # right outcome.
    statement = _one_month(
        ",5001,2026-01-01T10:00:00,Payment,Complete,T-Mobile Payment,"
        "Jordan Mercer,Alex Liang,- $35.00,,,,Venmo balance,,,",
        ",5002,2026-02-01T10:00:00,Payment,Complete,T mobile payment,"
        "Jordan Mercer,Alex Liang,- $35.00,,,,Venmo balance,,,",
        ",5003,2026-01-05T10:00:00,Payment,Complete,dinner,"
        "Jordan Mercer,Alex Liang,- $22.00,,,,Venmo balance,,,",
    )
    txs, _ = import_venmo(io.StringIO(statement), "acct1")

    # Punctuation and case differences must not split one series in two.
    assert txs[0].merchant == txs[1].merchant
    assert "T MOBILE" in txs[0].merchant
    assert "ALEX LIANG" in txs[0].merchant
    # A different note is a different thing, and groups separately.
    assert txs[2].merchant != txs[0].merchant


def test_a_short_note_leaves_the_counterparty_alone():
    # "gas" labels an occasion rather than naming a service, so folding it in
    # would fragment one person into many merchants for no gain.
    statement = _one_month(
        ",5004,2026-01-01T10:00:00,Payment,Complete,gas,"
        "Jordan Mercer,Sam Okafor,- $20.00,,,,Venmo balance,,,"
    )
    txs, _ = import_venmo(io.StringIO(statement), "acct1")
    assert txs[0].merchant == "SAM OKAFOR"
